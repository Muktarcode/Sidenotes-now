...
const SideNotesPanel = ({ isOpen, onClose, isDark, onThemeToggle }) => {
  ...
  return (
    <div className={`
      ...
      ${isFullscreen ? 'left-0 w-full right-0' : 'right-0'}
    `}>
      {/* Overlay */}
      {isOpen && !isFullscreen && (
        <div 
      ...
      `}>
        ...
        {/* Notes List Panel */}
        <div className={`
          ${isFullscreen ? 'w-1/3 min-w-[350px] border-r' : 'w-full'} 
          flex flex-col
        ...
        {selectedNote && (
          <div className={`
            ${isFullscreen ? 'flex-1' : 'hidden md:block'}
          `}>
            <NoteEditor
              note={selectedNote}
              onUpdate={handleNoteUpdate}
              onClose={() => !isFullscreen && setSelectedNote(null)}
              isDark={isDark}
              isOpen={isOpen}
